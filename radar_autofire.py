    ...
    def updateSystemSelect(self):
        if not self.updated:
            for (name,label) in self.systems:
                #print(f"dit zijn de {name,label}")
...
#             self.connection.sendMessage('SYSTEM  tsit-a-testrange 1 3')
    ...
    def calculate_FireAt(self):
        heading, (ground_distance, height) = self.active_target.posToBearing(self.active_target.futureposition)
        ...
        bearing = heading
        self.commando =  "FIREAT " + str(bearing) + ' ' + str(elevatie) + ' ' + str(self.active_target.lastupdate) + str((10*1000) - round(tijd*1000))
